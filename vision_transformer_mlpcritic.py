  ...
  def forward(self, *inputs):
    ...
    for i, net in enumerate(self.networks):
      if self.schema:
        ...
        pred_attns.append(pred_attn)
        h1ms.append(h1m)
      else:
        response = net(inputs)[-1]
        n_envs, output_dim = response.shape
        field_dim = int(math.sqrt(output_dim/2))
        response = response.reshape(n_envs,
          field_dim, field_dim, 2)
...
class MultiAgentMixed(nn.Module):
  def __init__(self, n_agents, n_agent_outputs,
...
class MultiAgentMlp(nn.Module):
  def __init__(self, n_agents, n_agent_outputs, n_chans, centralised, field_dim):
    super().__init__()
    self.n_agents = n_agents
    ...
    self.networks = nn.ModuleList(
        [Mlp(
          in_features=((n_chans)*field_dim**2),
  ...
  def forward(self, *inputs):
    ...
    for i, net in enumerate(self.networks):
      response = net(inputs)
      responses.append(response)
    # out: [n_envs, n_agents, 1, 1, 1]
    output = torch.stack(responses, dim=1).unsqueeze(-1).unsqueeze(-1)
    return output
