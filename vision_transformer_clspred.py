...
class VitAttentionSchema(nn.Module):
    def __init__(self, img_size=[256], patch_size=16, in_chans=3, num_classes=2, embed_dim=384, depth=6,
        ...
        self.pred_dim = h2_dim*(num_patches+1)
        ...
        self.predictor = Mlp(in_features=self.pred_dim, hidden_features=mlp_hidden_dim, out_features=embed_dim)
    ...
    def forward(self, x):
        ...
        pred_attn = self.predictor(h2.reshape(B, self.pred_dim))
        policy = self.policy(h1m[:, 0, :]) # use cls token for prediction label

        return pred_attn, h1m[:, 0, :], policy
